...
// Configurações de internet (usado apenas para sincronizar a hora)
const char* INTERNET_WIFI_SSID = "";
const char* INTERNET_WIFI_PASSWORD = "";
...
// Posição da antena
const double ANTENNA_LAT = -15.779;
const double ANTENNA_LON = -47.929;

WiFiClient espClient;
PubSubClient client(espClient);

// Cabeçalhos das funçoes
...
void setup_ap() {
  ...
  // Conecta a internet para sincronizar horario
  WiFi.mode(WIFI_STA);
  WiFi.begin(INTERNET_WIFI_SSID, INTERNET_WIFI_PASSWORD);
  Serial.print("\nConectando a rede ");
  Serial.println(INTERNET_WIFI_SSID);
  ...
  Serial.print("\nAguardando sincronizacao de tempo via NTP...");
  ...
  Serial.print("Hora atual para checagem: ");
  Serial.print(ctime(&now));

  //------------MUDANCA DE WIFI PARA MODO AP-------------------
  ...
  Serial.println(IPAddress(255, 255, 255, 0)); // Mascara fixa para /24
  Serial.print("Gateway: ");
  Serial.println(WiFi.softAPIP()); // O próprio AP é o gateway
  Serial.println("Confirme que quer continuar digitando qualquer tecla");
...
void handlePositionMessage(String droneId, JsonDocument& doc) {
  // Verificação da idade da mensagem
  unsigned long long drone_time = doc["timestamp"];
  unsigned long long antena_time = (unsigned long long)time(nullptr) * 1000;
  long long message_age = antena_time - drone_time;
  const long long MAX_MESSAGE_AGE_MS = 1800; // 2 segundos

  if (drone_time == 0 || message_age > MAX_MESSAGE_AGE_MS) {
    Serial.printf("[ANTENA] Mensagem do drone %s descartada por ser muito antiga (%lld ms).\n", droneId.c_str(), message_age);
    return; // Nao atualiza nada caso a mensagem seja velha demais
  }
...
void sendDroneCommand(String droneId, JsonDocument& message) {
  // Construir topico especifico para o drone, inserindo a variavel pelo seu pid
  char topic[50];
...
void setup() {
  ...
  // Define que a funcao executada ao chegar uma mensagem é callback
